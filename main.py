...
def obstacle_movement(obstacle_list):
    if obstacle_list:
        for obstacle_rect in list(obstacle_list):
            obstacle_rect.x -= 5
...
sky_rect = sky_surface.get_rect(topleft = (0, -330))

ground_surface = pygame.image.load('graphics/ground.png').convert()

# score_surf = test_font.render('My game', False, (64,64,64))
# score_rect = score_surf.get_rect(midtop = (screen_rect.centerx, 50))
...
start_rect = game_message.get_rect(center = (400, 330))
...
while True:
    for event in pygame.event.get():
        ...
        if game_active:
            if event.type == pygame.MOUSEBUTTONDOWN:
                if player_rect.bottom >= 300 and player_rect.collidepoint(event.pos): player_gravity = -25

            if event.type == pygame.KEYDOWN:
                if player_rect.bottom >= 300 and event.key == pygame.K_SPACE: player_gravity = -25
    ...
    if game_active:
        ...
        # if obstacle_rect_list == None: game_active = False
        ...
        if score == 0: screen.blit(game_message,start_rect)
